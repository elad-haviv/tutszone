    ...
    <section id="course-view">
        <div class="uk-grid uk-grid-collapse">
                    ...
                    @if(count($lessons) > 0)
                        ...
                        <ol class="uk-list uk-list-line uk-list-space" id="lessons-list">
                            @foreach($lessons as $lesson)
                                <li @if (isset($current))
                                    {!! $lesson->name === $current->name ? 'class="active"' : '' !!}
                                    @endif>
                                    ...
                                    @if(Auth::check())
                                        @if(Auth::user()->group == 1)
                                            <span class="uk-float-right">
                                                <a href="#" data-delete-lesson="{{ $lesson->id }}"><span class="fa fa-trash"></span></a>
                                                <a href="{{route('admin:lesson:edit',['id'=>$lesson->id])}}"><span class="fa fa-pencil"></span></a>
                                            </span>
                                        @endif
                                    @endif
                                </li>
            ...
            <div class="uk-width-medium-2-3" id="main-content">
                @if(isset($current))
                    ...
                    <div class="uk-panel uk-panel-box">
                        <form class="uk-form" action="{{ route("course:comment") }}" method="post">
                            ...
                            <button class="uk-button uk-button-success uk-width-1-1">{{ trans('lesson.add-comment') }}</button>
                        </form>
                        @if(!Auth::check())
                            <div class="uk-position-cover uk-overlay-background uk-overlay-panel uk-flex uk-flex-center uk-flex-middle uk-text-center">
                                <div>
                                    ...
                                    <p class="uk-margin-top">
                                        <a href="{{ route("auth:login") }}" class="uk-button uk-button-success">{{ trans("auth.login") }}</a>
                                        <a href="{{ route("auth:register") }}" class="uk-button uk-button-success">{{ trans("auth.register") }}</a>
                                    </p>
                                </div>
                            </div>
                        @endif
                ...
                @if(Auth::check())
                    @if(Auth::user()->group == 1)
                        <a href="{{ route("admin:lesson:add", ["data" => $course->id]) }}" title="{{trans('admin.course.add')}}" class="uk-button uk-button-large uk-button-success" style="width:100%">
                            <span class="fa fa-plus-circle"></span>
                            {{ trans("admin.lesson.add") }}
                        </a>
                    @endif
	...
	$(document).ready(function(){
		$(window).scroll(function() {
			if ($(this).scrollTop() > 150) {
				$('.to-top-button').stop().animate({bottom: '50px'}, "fast");
			} else {
				$('.to-top-button').stop().animate({bottom: '-100px'}, "fast");
			}
		});
        @if(Auth::check())
        @if(Auth::user()->group == 1)
        $('[data-delete-lesson]').click(function(){
            if(confirm("{{trans("admin.lesson.delete-confirm")}}") == true) {
                window.location = "{{route('admin:lesson:delete',['id'=>null])}}/" + $(this).attr('data-delete-lesson');
            }
        });
        @endif
        @endif
