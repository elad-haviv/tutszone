...
<li>
    <a href="{{ route("category:home") }}" title="{{ trans("nav.categories") }}" class="uk-navbar-nav-subtitle uk-text-center">
            ...
            <ul class="uk-nav uk-nav-navbar">
                @foreach($pages as $page)
                    <li>
                        @if(Auth::check())
                            @if(Auth::user()->group == 1)
                                <span class="uk-float-right" style="padding: 5px;">
                                    <a style="color: inherit;" href="#" data-delete-page="{{$page->id}}"><span class="fa fa-trash"></span></a>
                                    <a style="color: inherit;" href="{{route("admin:page:edit",['id' => $page->id])}}"><span class="fa fa-pencil"></span></a>
                                </span>
                            @endif
                        @endif
                        <a
                        @if($page->show == 0)
                            class="uk-text-muted"
                        @endif href="{{route("page", ["name" => $page->name])}}" title="{{$page->title}}">{{$page->title}}</a>
                    </li>
                @endforeach
                    @if(Auth::check())
                        @if(Auth::user()->group == 1)
                ...
                $(document).ready(function(){
                    $('[data-delete-page]').click(function(){
                        if(confirm("{{trans("admin.page.delete-confirm")}}") == true) {
                            window.location = "{{route('admin:page:delete',['id'=>null])}}/" + $(this).attr('data-delete-page');
                        }
                    });
                });
            </script>
