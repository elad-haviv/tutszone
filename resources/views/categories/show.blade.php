    ...
    <div id="category-view">
        <div class="uk-grid uk-grid-collapse">
                            ...
                            @foreach ($subCategories as $subCategory)
                                <li>
                                    <a href="{{ route('category:show', ['name' => $subCategory->name]) }}"
                                        data-uk-tooltip="" title="{{ $subCategory->description }}">
                                        {{ $subCategory->title }}
                                    </a>
                                    @if (Auth::check())
                                        @if (Auth::user()->group == 1)
                                            <div class="uk-float-right">
                                                <a href="#" data-delete-category="{{ $subCategory->id }}"><span
                                                        class="fa fa-trash"></span></a>
                                                <a href="{{ route('admin:category:edit', ['id' => $subCategory->id]) }}"><span
            ...
            <div class="uk-width-medium-2-3" id="main-content">
                                ...
                                <div class="uk-grid">
                                    <div class="uk-width-medium-1-5 uk-text-center padded">
                                        <a href="{{ route('course:show', ['name' => $course->name]) }}">
                                            <img class="uk-border-circle" alt="course thumbnail"
                                                src="{{ url($course->picture ? $course->picture : 'images/logo.svg') }}" />
                                        </a>
                                    </div>
                                    <div class="uk-width-medium-4-5 uk-text-center uk-margin">
                                        <h2>
                                            <a
                                                href="{{ route('course:show', ['name' => $course->name]) }}">{{ $course->title }}</a>
                                            @if (Auth::check())
                                                @if (Auth::user()->group == 1)
                                                    <span class="uk-float-right">
                                                        <a href="#" data-delete-course="{{ $course->id }}"><span
                                                                class="fa fa-trash"></span></a>
                                                        <a href="{{ route('admin:course:edit', ['id' => $course->id]) }}"><span
                                                                class="fa fa-pencil"></span></a>
                                                    </span>
                                                @endif
                                            @endif
                                    ...
                                    <a href="{{ route('category:show', ['name' => $subCategory->name]) }}">
                                        <div class="uk-panel uk-panel-box tz-category-card">
                                            <div class="uk-margin-bottom">
                                                <img src="{{ url($subCategory->picture !== null ? $subCategory->picture : 'resources/assets/images/default-category.png') }}"
                ...
                @if (Auth::check())
                    @if (Auth::user()->group == 1)
                        <a href="{{ route('admin:course:add', ['data' => $category->id]) }}"
                            title="{{ trans('admin.course.add') }}" class="uk-button uk-button-large uk-button-success"
                            style="width:100%">{{ trans('admin.course.add') }}</a>
                    @endif
                @endif
            </div>
        </div>
                ...
                $(document).ready(function() {
                    ...
                    $('[data-delete-course]').click(function() {
                        if (confirm("{{ trans('admin.course.delete-confirm') }}") == true) {
                            window.location = "{{ route('admin:course:delete', ['id' => null]) }}/" + $(this).attr(
                                'data-delete-course');
                        }
                    });
                });
