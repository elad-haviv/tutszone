...
@section("content")
                ...
                @if(Auth::user()->group == 1)
                    <div class="uk-width-medium-1-3 uk-margin-top">
                        <a href="#add-category-form" data-uk-modal="{center:true}">
                            <div class="uk-panel uk-panel-box tz-category-card uk-flex uk-flex-center uk-flex-middle uk-text-center">
                                <span class="fa fa-plus-circle tz-text-xl"></span>
                            </div>
                        </a>
                        @include("admin.add_category",["show"=>"modal", "id" => "add-category-form", "defaultPicture" => 'resources/assets/images/default-category.png'])
                    </div>
                @endif
            @endif
                    ...
                    <a href="{{ route("category:show", ["name" => $category->name]) }}">
                        <div class="uk-panel uk-panel-box tz-category-card">
                            <div class="uk-margin-bottom">
                                <img src="{{ url($category->picture !== null ? $category->picture : "resources/assets/images/default-category.png") }}" alt="thumbnail" title="thumbnail" class="uk-border-circle" />
                            </div>
                    ...
                    @if(Auth::check())
                        @if(Auth::user()->group == 1)
                            <div class="uk-position-top-right">
                                <a href="#" data-delete-category="{{ $category->id }}" class="uk-button uk-button-danger tz-radius-sm"><span class="fa fa-trash"></span></a>
                                <a href="{{route('admin:category:edit',['id'=>$category->id])}}" class="uk-button uk-button-success tz-radius-sm"><span class="fa fa-pencil"></span></a>
                            </div>
                        @endif
                    @endif
                </div>
    ...
    <div class="uk-container uk-container-center uk-margin-large">
        {!! $categories->render(new \TutsZone\Pagination\UiKitPresenter($categories)) !!}
    </div>
@endsection
