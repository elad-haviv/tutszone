...
@section("title")
    @parent - {{ trans("admin.category.edit") }}
@endsection

@section("content")
    <div class="uk-block uk-block-muted uk-margin-large-top">
        <h1 class="uk-panel-title uk-text-center">{{trans('admin.category.edit')}}&colon; {{$category->title}}</h1>
        <form class="uk-form uk-form-horizontal uk-container" method="post" action="{{ route("admin:category:post.edit") }}">
            {!! csrf_field() !!}
            @if (count($errors) > 0)
            ...
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <input id="title" name="title" type="text" class="uk-width-1-1" value="{{$category->title}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="name">{{ trans('admin.category.name') }}</label>
                <div class="uk-form-controls">
                    <input id="name" name="name" type="text" class="uk-width-1-1" value="{{$category->name}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="parent">{{ trans('admin.category.parent') }}</label>
                <div class="uk-form-controls">
                    <select id="parent" name="parent" class="uk-width-1-1">
                        {!! $parentSelector !!}
                    </select>
                </div>
            </div>
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <textarea id="description" name="description" class="uk-width-1-1 disable-resize" style="height: 150px;">{{$category->description}}</textarea>
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="picture">{{ trans('admin.category.picture') }}</label>
                <div class="uk-form-controls uk-grid">
                    <input id="picture" name="picture" type="text" class="uk-width-7-10" value="{{$category->picture}}" />
            ...
            <div class="uk-modal-footer uk-text-right">
                <button class="uk-button uk-button-primary" type="submit">{{ trans('admin.category.edit') }}</button>
            </div>
        </form>
    </div>
