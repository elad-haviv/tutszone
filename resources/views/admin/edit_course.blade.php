...
@section("content")
    <div class="uk-block uk-block-muted uk-margin-large-top">
        <h1 class="uk-panel-title uk-text-center">{{trans('admin.course.edit')}}: {{$course->title}}</h1>
        <form class="uk-form uk-form-horizontal uk-container" method="post" action="{{ route("admin:course:post.edit") }}">
            {!! csrf_field() !!}
            @if (count($errors) > 0)
            ...
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <input id="title" name="title" type="text" class="uk-width-1-1" value="{{$course->title}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="name">{{ trans('admin.course.name') }}</label>
                <div class="uk-form-controls">
                    <input id="name" name="name" type="text" class="uk-width-1-1" value="{{$course->name}}" />
                </div>
            </div>
            <div class="uk-form-row">
            ...
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <textarea id="description" name="description" class="uk-width-1-1 disable-resize" style="height: 150px;">{{$course->description}}</textarea>
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="picture">{{ trans('admin.course.picture') }}</label>
                <div class="uk-form-controls uk-grid">
                    <input id="picture" name="picture" type="text" class="uk-width-7-10" value="{{$course->picture}}" />
            ...
            <div class="uk-modal-footer uk-text-right">
                <button class="uk-button uk-button-primary" type="submit">{{ trans('admin.course.edit') }}</button>
            </div>
        </form>
    </div>
