...
@section("title")
    @parent - {{ trans("admin.lesson.add") }}
@endsection

@section("content")
    <div class="uk-block uk-block-muted uk-margin-large-top">
        <h1 class="uk-panel-title uk-text-center">{{trans('admin.lesson.edit')}} &quot;{{$lesson->title}}&quot;</h1>
        <form class="uk-form uk-form-horizontal uk-container" method="post" action="{{ route("admin:lesson:post.edit") }}">
            {!! csrf_field() !!}
            @if (count($errors) > 0)
            ...
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <input id="title" name="title" type="text" class="uk-width-1-1" value="{{$lesson->title}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="name">{{ trans('admin.lesson.name') }}</label>
                <div class="uk-form-controls">
                    <input id="name" name="name" type="text" class="uk-width-1-1" value="{{$lesson->name}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="addons">{{ trans('admin.lesson.addons') }}</label>
                <div class="uk-form-controls">
                    <input id="addons" name="addons" type="text" class="uk-width-1-1" value="{{$lesson->addons}}" />
                </div>
            </div>
            <div class="uk-form-row">
                <label class="uk-form-label" for="picture">{{ trans('admin.lesson.picture') }}</label>
                <div class="uk-form-controls uk-grid">
                    <input id="picture" name="picture" type="text" class="uk-width-7-10" value="{{$lesson->picture}}" />
            ...
            <div class="uk-form-row">
                ...
                <div class="uk-form-controls">
                    <textarea id="lead" name="lead" class="uk-width-1-1 disable-resize" style="height: 150px;">{{$lesson->lead}}</textarea>
                </div>
            </div>
            <div class="uk-form-row">
            ...
            <div class="uk-form-row">
                <textarea id="lesson-content" name="content" class="uk-width-1-1 disable-resize" style="height: 150px;">{{$lesson->content}}</textarea>
            </div>
            <div class="uk-modal-footer uk-text-right">
                <button class="uk-button uk-button-primary" type="submit">{{ trans('admin.lesson.edit') }}</button>
            </div>
        </form>
    </div>
